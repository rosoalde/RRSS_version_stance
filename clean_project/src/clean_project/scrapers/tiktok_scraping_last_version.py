...
def convertir_fecha(fecha):
    ...
    fecha_convertida = dt.strftime("%d/%m/%Y")

    return fecha_convertida

def busqueda_google(driver, config, query):
    after = convertir_fecha(str(config.general["end_date"]))
    before = convertir_fecha(str(config.general["start_date"]))

    clean_query = query.replace('"', '')
    encoded_query = urllib.parse.quote(clean_query)
    encoded_query = f'"{encoded_query}"'

    search_box = driver.find_element(By.NAME, "q")
    time.sleep(3)
    search_box.send_keys("pruebaas de tiki toko")
    time.sleep(3)
    search_box.send_keys(Keys.RETURN)
    
    #before = "01/12/2018"
    #after = "02/16/2020"
    url = f'https://www.google.com/search?q=site:tiktok.com+{encoded_query}+inurl:video+&tbs=cdr:1,cd_min:{before},cd_max:{after}'
    #url = f'https://www.google.com/search?q=site:tiktok.com+%22crisis%22+inurl:video+&tbs=cdr:1,cd_min:26/11/2025,cd_max:09/12/2025&udm=7'
...
def recopilar_respuestas(driver):
    ...
    print("Respuestas a un comentario----------------")    
...
def recopilar_comentarios(driver):
    ...
    for comment in comments:
        ...
        print("comentarios--------------------")
        print(f"Respuestas a un comentario: {data}")
...
def scrape_tiktok(config):
    ...
    # 2️⃣ Buscar TikTo
    query = config.scraping["tiktok"]["query"][0]
    # query="crisis"
    busqueda_google(driver, config, query)
