...
def metadatos_video(driver):
    ...
    try:
        ...
        print("fallo keywords --- 2 punto 0")
        keywords = ""

    try:
        fecha = driver.find_element(
            By.XPATH,
            "//div[contains(@class,'DivCreatorInfoContainer')]//span[contains(text(),'·')]"
        ).text
    except NoSuchElementException:
        print("fallo fecha")
        fecha = ""
    
    fecha = normalizar_fechas(fecha)
    after = config.general["end_date"]
    before = config.general["start_date"]
    fecha_publicacion_formato= datetime.strptime(fecha, "%Y-%m-%d")
    if fecha_publicacion_formato < datetime.strptime(before, "%Y-%m-%d") or fecha_publicacion_formato > datetime.strptime(after, "%Y-%m-%d"):
        x = "deberia salir"
        return {}
    else:
        x = "fallo"
...
def recopilar_respuestas(driver):
    ...
    for respuesta in respuestas:
        ...
        try:
            ...
            after = config.general["end_date"]
            before = config.general["start_date"]
            fecha_publicacion_formato= datetime.strptime(fecha_publicacion, "%Y-%m-%d")
            if fecha_publicacion_formato < datetime.strptime(before, "%Y-%m-%d") or fecha_publicacion_formato > datetime.strptime(after, "%Y-%m-%d"):
                continue
...
def scrape_video(driver):
    ...
    resultado = []
    metadata_perfil = {}  # <-- inicialización segura
    main_tab = driver.current_window_handle
    metadata_video = metadatos_video(driver)
    if not metadata_video:
        metadata_video = {
            "titulo": "",
            "likes": "",
            "comentarios": "",
...
def guardar_datos(datos, output_folder):#(datos, archivo="tiktok_dataset.csv"):
    """
    ...
    output_file = f"{output_folder}/tiktok_global_dataset.csv"
...
def scrape_datos(driver, config, limit=10):
    ...
    for url in urls:
        ...
        print(f"output_folder en scrape_datos: {config.general['output_folder']}")
        print("Guardando datos...")
        output_folder = config.general["output_folder"]
        guardar_datos(datos, output_folder)
        print(f"output_folder después de guardar_datos: {config.general['output_folder']}")
...
def run_tiktok(config):
    
    output_folder = config.general["output_folder"]
    
    Path(output_folder).mkdir(parents=True, exist_ok=True)
    output_file = f"{output_folder}/tiktok_global_dataset.csv"
    print("Iniciando TikTok Scraper...")
    print(f"Ruta del archivo de salida: {output_file}")
    # Si el archivo ya existe, saltamos el scraper
    if Path(output_file).exists() and Path(output_file).stat().st_size > 0:
        print(f"⚠️ El archivo CSV de TikTok ya existe en: {output_file}. Saltando este scraper...")
